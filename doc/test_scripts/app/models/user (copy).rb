...
class User < ActiveRecord::Base
  ...
  has_many :ownerships
  has_many :owners, :through => :ownerships,:conditions =>["ownable_type=?",'user'] ,:source=>:user,:include =>[:ownerships]

  has_many :inverse_ownerships, :class_name => "Ownership", :foreign_key => "ownable_id"
  has_many :inverse_owners, :through => :inverse_ownerships, :source => :user ,:conditions =>["ownable_type=?",'user'] ,:include =>[:ownerships]
	       ...
	       self.mrcs
						  if !@the_mrcs.blank?
							  	@the_mrcs.each do |mrc|
								  	@the_clients << mrc.clients
								    set_roles(@the_clients,mrc.current_user_right)
								  end
					   	end
				    @the_clients <<  owned_clients	
		    else
			    role = self.my_role
	   ...
	   if self.is_user?
			 ...
			 if !@the_clients.blank?
					@the_clients.each do |client|
						@the_projects << client.projects
						set_roles(@the_projects,client.current_user_right)
					end
				end
				@the_projects << owned_projects
	  else
			role = self.my_role
	...
	def populate_users(items)
		if !items.blank?
		   items.each do |item|
				 ...
				 set_roles(@the_users,item.current_user_right)
				end
	  end
	end
